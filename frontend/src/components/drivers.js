...
function ShowAccounts(status) {
    ...
    return (
        console.log(status),
        <div>
            <h2>Driver Id, Vehicle Id</h2>
            {status.status._id ? <div key={status.status._id}>
                <h3>{status.status._id}, {status.status.vehicle_id}</h3>
            ...
            {Array.from(status.status).map((item) => (
                <div key={item._id}>
                    <h3>{item._id}, {item.vehicle_id}</h3>
...
function Drivers() {
    ...
    return (
        ...
        <div>
            ...
            <br/><br/><br/>
            <div className="special"><button onClick={() => {handleClick(setStatus, id)}}>Search drivers</button></div>
